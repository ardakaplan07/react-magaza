...
function AdminDashboard({ onLogout, user }) {
  ...
  const handleBulkAdd = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/admin/bulk-add-products', {
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/admin/delete-product/${productId}`, { method: 'DELETE' });
  ...
  const handleApplyDiscount = async (productId) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/admin/update-discount/${productId}`, {
  ...
  const handleToggleProductFlag = async (productId, flagName, currentValue) => {
    try {
      const response = await fetch(`http://localhost:5000/api/admin/update-product-flags/${productId}`, {
  ...
  const handleToggleRole = async (userId, currentRole) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/admin/update-role/${userId}`, {
  ...
  const saveCategory = async (productId) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/admin/update-category/${productId}`, {
...
function StoreFront({ onLogout, user }) {
  ...
  const handleInitiate3DSecure = async (e) => {
    ...
    try {
        await fetch('http://localhost:5000/api/payment/send-sms', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
  ...
  const handleVerify3DSecure = async (e) => {
    ...
    try {
        const verifyResponse = await fetch('http://localhost:5000/api/payment/verify-sms', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
